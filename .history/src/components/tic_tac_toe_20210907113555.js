...
export default function Tic_Tac_Toe(props) {
  ...
  const cb_check_for_winner = () => {
    console.log('Array.from(stateCircle).length = ', Array.from(stateCircle).length)
    console.log('Array.from(stateXray).length = , ', Array.from(stateXray).length)
  ...
  if (changedCell) {
    ...
    temp_board_state[index].state = statePlayerTurn ? "Circle" : "X-ray";
